...
2. run `migrate history` to see the recorded end state
3. if the local migration is the next reviewed change, run `migrate apply`
...
- `/docs/migrations/cli-commands` for exact command usage
- `/docs/migrations/rollback-and-recovery` for recovery strategy
- `/docs/migrations/flutter-local-sqlite` when the problem is device-local upgrade code rather than shared-database rollout
